...
namespace MuSoeun
{
    class Object {
    public:
        int CoordX = 10, CoordY = 5;    //오브젝트 좌표
    ...
    private:
        ...
        int width = 50, height = 20;    //게임 스크린의 너비, 높이

    public:
        int score;                  //현재 게임 점수 저장
        int speed;                  //현재 게임 속도 저장

        vector<Snake> snakeLine;    //뱀 (한 칸당 리스트 요소 한 개씩)
        vector<Apple> Apples;       //사과 개수 저장
        Snake snake;
        Apple apple;
        ...
        void Draw() {
            ...
            for (int i = 0; i < snakeLine.size(); i++) { //스크린, 뱀의 좌표, 스크린 가로 길이를 매개변수로
                //꼬리부터 몸통 출력
        ...
        void ObjectCreate(int setSummonApple) {
            snakeLine.clear(); Apples.clear();      //뱀, 사과 저장 데이터 삭제
            snake.CoordX = 10; snake.CoordY = 10;   //뱀의 초기 위치 지정

            //뱀 위치 저장
            for (int i = 0; i < 5; i++)
            {
                snake.CoordX++; snakeLine.push_back(snake);
            }

            //플레이어가 설정한 사과 개수 만큼 사과 (+위치) 저장
        ...
        int Move(int SnakeMove, int keyInput) {
            //뱀 이동 방향 기준 좌우 방향키 입력에 따른 뱀 이동 방향 전환
            switch (keyInput)
            {
            case Key_Left:
                switch (SnakeMove)
        ...
        int Crash(int x, int y) {
            //뱀 머리가 벽에 닿거나 뱀의 몸에 닿으면 충돌 값 반환 (0 : 충돌 / 1 : 미충돌)
    ...
    public:
        ...
        int Run(int setSpeed, int setSummonApple) {
            ...
            score = 0; speed = setSpeed;//점수, 속도 초기화(설정한 속도를 저장)

            //스크린 화면 초기화 후 오브젝트 초기화
            ClearScreenBuf();
